...
echo checking jq:
command -v jq || { echo "install jq first (https://jqlang.org/download/)"; }
echo checking step:
command -v step || { echo "install step first (https://smallstep.com/docs/step-cli/installation/)"; }
...
if ! test -f mds.jwt; then
    ...
    openssl verify -CAfile /etc/ssl/cert.pem -untrusted intermediates.pem mds.pem
